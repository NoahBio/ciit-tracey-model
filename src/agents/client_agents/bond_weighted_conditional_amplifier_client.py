"""Bond-weighted conditional frequency-amplifier expectation mechanism."""
...
class BondWeightedConditionalAmplifier(ConditionalAmplifierClient):
    """
    Conditional frequency amplifier with bond-modulated history influence.

    Extends ConditionalAmplifierClient by scaling history_weight by bond level:
    ...
    2. Calculate P(therapist_j | client_i) with Laplace smoothing
    3. Amplify: adjusted[i,j] = U[i,j] + (U[i,j] × P(j|i) × effective_weight)
