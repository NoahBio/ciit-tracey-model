"""Conditional frequency-filter expectation mechanism."""
...
class ConditionalFilterClient(BaseClientAgent):
    """
    Conditional frequency as filter: adjusted[i,j] = U[i,j] × P(j|i)

    History creates conditional probability distribution P(therapist_j | client_i).
    Unlike marginal frequency_filter, this tracks how the therapist responds to
    EACH specific client action, preventing degenerate distributions.
    ...
    3. Filter utilities: adjusted[i,j] = U[i,j] × P(j|i)
    4. Sort adjusted utilities for client action i
    5. Select percentile based on bond level

    Properties:
    - More robust than marginal frequency_filter
    - Smoothing prevents complete zero-out
    - Each client action has its own conditional distribution
    - Degrades gracefully when client action rarely observed
    ...
    def _calculate_expected_payoffs(self) -> NDArray[np.float64]:
        """
        Calculate expected payoffs using conditional frequency filtering.
        ...
        for client_action in range(8):
            ...
            # Filter: multiply utilities by conditional probability
            adjusted_utilities = raw_utilities * conditional_freq
