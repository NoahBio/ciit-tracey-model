...
import pytest
import numpy as np
from src.agents.client_agents import FrequencyAmplifierClient, create_problematic_client, BaseClientAgent
from src.config import MEMORY_SIZE, U_MATRIX, sample_u_matrix

# For backward compatibility, alias FrequencyAmplifierClient as ClientAgent
# (this matches the behavior of the legacy client_agent.py)
ClientAgent = FrequencyAmplifierClient
...
class TestClientAgent:
    ...
    def test_initialization_invalid_memory_length(self):
        ...
        with pytest.raises(ValueError, match="must have length"):
            ClientAgent(u_matrix=u_matrix, entropy=0.5, initial_memory=memory)
    ...
    def test_relationship_satisfaction_calculation(self):
        ...
        client = ClientAgent(u_matrix=U_MATRIX, entropy=0.5, initial_memory=memory)
    ...
    def test_bond_monotonicity(self):
        ...
        client_poor = ClientAgent(u_matrix=U_MATRIX, entropy=0.5, initial_memory=poor_memory)

        # Good relationship
        good_memory = [(0, 4)] * MEMORY_SIZE  # D-S (complement)
        client_good = ClientAgent(u_matrix=U_MATRIX, entropy=0.5, initial_memory=good_memory)
    ...
    def test_expected_payoffs_bond_effect(self):
        ...
        client_high_bond = ClientAgent(u_matrix=U_MATRIX, entropy=0.5, initial_memory=good_memory)

        # Low bond client (poor history)
        poor_memory = [(0, 0)] * MEMORY_SIZE  # D-D (very negative)
        client_low_bond = ClientAgent(u_matrix=U_MATRIX, entropy=0.5, initial_memory=poor_memory)
    ...
    def test_softmax_temperature_effect(self):
        ...
        client_low_ent = ClientAgent(u_matrix=U_MATRIX, entropy=0.1, initial_memory=memory, random_state=42)
        payoffs = client_low_ent._calculate_expected_payoffs()
        probs_low = client_low_ent._softmax(payoffs)

        # High entropy (random)
        client_high_ent = ClientAgent(u_matrix=U_MATRIX, entropy=2.0, initial_memory=memory, random_state=42)
    ...
    def test_action_selection_stochastic(self):
        ...
        memory = [(0, 4)] * MEMORY_SIZE
        client = ClientAgent(u_matrix=U_MATRIX, entropy=1.0, initial_memory=memory, random_state=42)
    ...
    def test_generate_problematic_memory(self):
        ...
        for pattern in ["cold_stuck", "dominant_stuck", "submissive_stuck"]:
            memory = BaseClientAgent.generate_problematic_memory(
    ...
    def test_create_client_convenience(self):
        ...
        client = create_problematic_client(pattern_type="cold_stuck", random_state=42)

        assert isinstance(client, BaseClientAgent)
        assert len(client.memory) == MEMORY_SIZE
