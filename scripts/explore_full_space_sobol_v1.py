"""Sobol quasi-random sweep for full parameter space fANOVA estimation.

Unlike the TPE optimization studies, this script uses a Sobol quasi-random
sampler to achieve uniform coverage of the full 11-dimensional parameter space.
This is specifically designed to feed unbiased data into fANOVA, answering:

    "Which parameters determine whether strategic advantage is possible at all,
     across the full theoretical assumption space?"

This is complementary to the TPE study, which answers a different question:
    "Within the viable region, what drives how large the advantage is?"

The QMCSampler with Sobol sequences fills high-dimensional space more evenly
than pure random sampling. Optuna recommends this for coverage-oriented studies.
Target: 1024 trials (2^10, optimal for Sobol sequences) at 100 seeds per trial.
...
Explored parameters (identical range to TPE study):
- initial_memory_pattern, baseline_accuracy, threshold, bond_alpha,
  bond_offset, recency_weighting_factor, max_sessions, perception_window,
  seeding_benefit_scaling, skip_seeding_accuracy_threshold,
  quick_seed_actions_threshold, abort_consecutive_failures_threshold

Example usage:

# Run 1024 trials with 100 seeds each (default — high-confidence fANOVA)
python scripts/explore_full_space_sobol_v1.py \\
  --n-trials 1024 \\
  --study-name sobol_full_space_v1

# View in browser:
optuna-dashboard sqlite:///optuna_studies/sobol_full_space_v1.db
"""
...
def objective(trial: optuna.Trial, n_seeds: int = 50, therapist_version: str = 'v2', n_workers: int | None = None) -> float:
    ...
    pattern = suggest_parameter(trial, 'initial_memory_pattern')
    ...
    trial.set_user_attr('pattern', pattern)
...
def create_or_load_study(
    ...
    """Create new study or load existing one with Sobol quasi-random sampler.
    ...
    study = optuna.create_study(
        ...
        sampler=optuna.samplers.QMCSampler(
            qmc_type='sobol',
            seed=42,
            scramble=True,              # recommended for higher-dimensional spaces
            warn_independent_sampling=False,  # categorical params fall back to RandomSampler by design
        )
    )
...
def export_results(study: optuna.Study, output_dir: Path, top_k: int = 15):
    ...
    for i, config in enumerate(best_configs[:5]):
        ...
        print(f"  Pattern: {config['parameters'].get('initial_memory_pattern', 'N/A')}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Sobol quasi-random sweep for full parameter space fANOVA estimation",
    ...
    print(f"{'='*80}")
    print(f"SOBOL FULL SPACE EXPLORATION (fANOVA COVERAGE SWEEP)")
    print(f"{'='*80}")
    print(f"Study name: {args.study_name}")
    print(f"Database: {storage_path}")
    print(f"Trials: {args.n_trials} (target: 1024 for optimal Sobol coverage)")
    print(f"Seeds per trial: {args.n_seeds} (high-confidence setting — reliable fANOVA under conservative SNR assumptions)")
    print(f"Sampler: QMCSampler (Sobol, scramble=True)")
    print(f"{'='*80}\n")
    ...
    print(f"\n{'='*80}")
    print(f"SOBOL SWEEP COMPLETE")
    print(f"{'='*80}")
    print(f"\nTo visualize results in browser:")
    print(f"  optuna-dashboard sqlite:///{storage_path}")
    print(f"\nfANOVA importance scores reflect full parameter space coverage.")
    print(f"Compare with TPE study importances to distinguish gate-keepers from amplifiers.")
