...
complementary therapy. Unlike the single-pattern variant, initial_memory_pattern is
treated as a categorical search variable so the optimizer explores which pattern the
therapist is most effective for.
...
- Pattern: initial_memory_pattern (cold_stuck, dominant_stuck, submissive_stuck)
...
python scripts/optimize_omniscient_v2_advantage_across_memory_types.py \
  --n-trials 100 \
  --study-name freq_amp_v2_across_patterns \
  --n-seeds 50 \
  --therapist-version v2

# View results in browser:
optuna-dashboard sqlite:///optuna_studies/freq_amp_v2_across_patterns.db
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Optimize frequency_amplifier with OmniscientStrategicTherapist V2 across memory pattern types",
    ...
    parser.add_argument(
        '--study-name', '-s',
        type=str,
        default='freq_amp_v2_across_patterns',
    ...
    print(f"{'='*80}")
    print(f"FREQUENCY AMPLIFIER V2 OPTIMIZATION (ACROSS MEMORY TYPES)")
    print(f"{'='*80}")
